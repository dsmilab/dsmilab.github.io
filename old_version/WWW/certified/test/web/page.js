...
function toPic(){
		...
		if (document.getElementById( "orientation" ).value == "8"){
			...
			ctx.rotate(90 * Math.PI / 180);
			ctx.drawImage(imageDown, 0, -goalWidth, goalHeight, goalWidth*3);
			ctx.rotate(-90 * Math.PI / 180);
			ctx.drawImage(imageUp, 0, 0, goalWidth, goalHeight);
...
function fBrowserRedirect(){
    ...
    var isIE11 = navigator.userAgent.search("Trident") > -1;

    console.log(navigator.userAgent);
    if (isIE || isIE11) {
        //alert("本網站不支援Internet Explorer瀏覽器，建議使用chrome或FireFox瀏覽器，以呈現最佳效果");
    }
    if(bIsIpad){
    ...
    if(bIsMidp||bIsUc7||bIsUc||bIsCE||bIsWM){
        ...
        alert("建議使用chrome瀏覽器觀看，拍照功能請使用手機操作，以呈現最佳效果");
    }
}
